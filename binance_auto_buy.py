...
# ============ 配置 ============
CONFIG_FILE = Path.home() / '.openclaw/workspace/config/binance_auto_buy.json'
STATE_FILE = Path.home() / '.openclaw/workspace/data/binance_buy_state.json'
HISTORY_FILE = Path.home() / '.openclaw/workspace/data/binance_buy_history.jsonl'
...
class AutoBuyStrategy:
    ...
    def get_z_value(self):
        ...
        """
        # 方法1: 从看板获取（如果有 API）
        if self.config.get('z_api_url'):
            try:
                response = requests.get(self.config['z_api_url'], timeout=10)
                if response.status_code == 200:
                    data = response.json()
                    return data.get('z_value')
            except Exception as e:
                print(f"⚠️ 从API获取z值失败: {e}")
        
        # 方法2: 手动配置（临时方案）
        if self.config.get('manual_z_value') is not None:
            return self.config['manual_z_value']
        
        print("❌ 无法获取 z 值，请配置 z_api_url 或 manual_z_value")
        return None
